...
class SceneFilterDialog(GeometryManagerMixin, QDialog):
    ...
    def setup_ui(self):
        ...
        self.reset_favorites_button = QPushButton("Reset All Favorites")
        ...
        self.any_radio = QRadioButton("Match Any Category (OR)")
        self.all_radio = QRadioButton("Match All Categories (AND)")
        match_layout.addWidget(self.any_radio)
        match_layout.addWidget(self.all_radio)
        ...
        if self.orientations:
            ...
            self.orientation_list.setSelectionMode(QListWidget.SelectionMode.NoSelection)
        ...
        # --- Participant Filtering (for actions only) ---
        if self.mode == 'action':
            parts_group = QGroupBox("Filter by Participants")
            parts_layout = QHBoxLayout(parts_group)
            self.min_spin = QSpinBox(); self.min_spin.setRange(1, 20); self.min_spin.setValue(1)
            self.max_spin = QSpinBox(); self.max_spin.setRange(1, 20); self.max_spin.setValue(10)
            parts_layout.addWidget(QLabel("Min:"))
            parts_layout.addWidget(self.min_spin)
            parts_layout.addStretch()
            parts_layout.addWidget(QLabel("Max:"))
            parts_layout.addWidget(self.max_spin)
            main_layout.addWidget(parts_group)

        # --- Buttons ---
        button_box = QDialogButtonBox(QDialogButtonBox.StandardButton.Ok | QDialogButtonBox.StandardButton.Cancel)
        button_box.accepted.connect(self._apply_filters)
    ...
    def load_filters(self):
        ...
        for i in range(self.category_list.count()):
            ...
            if item.text() in selected_categories:
                item.setCheckState(Qt.CheckState.Checked)

        if self.current_filters.get('match_mode', 'any') == 'all':
        ...
        if self.orientations:
            ...
            for i in range(self.orientation_list.count()):
                ...
                if item.text() in selected_orientations:
                    item.setCheckState(Qt.CheckState.Checked)
    ...
    def _apply_filters(self):
        ...
        if self.orientations:
            ...
            for i in range(self.orientation_list.count()):
                item = self.orientation_list.item(i)
                if item.checkState() == Qt.CheckState.Checked:
                    selected_orients.append(item.text())
    ...
    def get_filters(self) -> dict:
        """Returns the filters that were applied when the user clicked OK."""
        return self.applied_filters

    def _reset_favorites(self):
        reply = QMessageBox.question(self, "Reset Favorites",
                                     f"Are you sure you want to reset all your favorite {self.mode} tags?\nThis action cannot be undone.",
                                     QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
                                     QMessageBox.StandardButton.No)
        if reply == QMessageBox.StandardButton.Yes:
            self.controller.reset_favorite_tags(self.mode)