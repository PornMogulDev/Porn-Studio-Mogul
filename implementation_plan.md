...
**Tour Sponsorship Streamlining**: Move the entire tour flow into this presenter:

| Currently In | Moves To | Action |
|-------------|----------|--------|
| `TalentProfileWindow._on_sponsor_tour_requested()` | `HiringPresenter._on_sponsor_tour_requested()` | **Move** — presenter now creates/shows `SponsorTourDialog` |
| `TalentProfileWindow.tour_sponsorship_confirmed` signal | *Deleted* | **Remove** — no longer needed |
| `TalentProfilePresenter.get_tour_sponsorship_preview()` | `HiringPresenter._get_tour_preview()` | **Move** (becomes private) |
| `TalentProfilePresenter._on_tour_sponsorship_confirmed()` | `HiringPresenter._execute_tour()` | **Move** (inline after dialog accepted) |

```python
class HiringPresenter(QObject):
    """Handles hiring, contracts, and tour sponsorship for a talent."""
    
    def __init__(self, controller: IGameController, widget: HiringWidget, view_for_dialogs: QWidget, parent=None):
        super().__init__(parent)
        self.controller = controller
        self.widget = widget
        self._dialog_parent = view_for_dialogs  # For SponsorTourDialog parenting
        self._current_talent: Optional[Talent] = None
        self._connect_signals()
    
    def _on_sponsor_tour_requested(self, roles_for_tour: list):
        """Presenter now owns the entire tour sponsorship flow."""
        if not self._current_talent:
            return
        
        # 1. Get preview from controller
        preview = self.controller.get_tour_sponsorship_preview(self._current_talent.id, roles_for_tour)
        if not preview.is_feasible:
            QMessageBox.warning(self._dialog_parent, "Tour Infeasible", preview.refusal_reason)
            return
        
        # 2. Show dialog (presenter creates it, not view)
        dialog = SponsorTourDialog(self._current_talent.alias, asdict(preview), self._dialog_parent)
        
        # 3. Handle result
        if dialog.exec():
            tour_details = dialog.get_selected_tour_details()
            self.controller.sponsor_tour(self._current_talent.id, roles_for_tour, tour_details, dialog.get_final_cost())
```
...
- **Remove tour sponsorship methods** (`get_tour_sponsorship_preview`, `_on_tour_sponsorship_confirmed`) — moved to `HiringPresenter`
...
#### [MODIFY] [talent_profile_view.py](file:///c:/Users/Gen/Documents/PSM/Game/hire_talent/0.4.6/src/ui/views/talent_profile_view.py) (Phase 2 cleanup)

Remove tour-related items (view becomes "dumber"):
- **Delete** `_on_sponsor_tour_requested()` method
- **Delete** `tour_sponsorship_confirmed` signal
- **Delete** `SponsorTourDialog` import
...
| 4 | Select 2+ non-local roles within 4 weeks, click "Sponsor Tour" | Tour dialog opens, completing it executes tour |
| 5 | In Chemistry widget, click another talent's name | Second talent's profile opens (cross-widget navigation works) |
| 6 | Change theme in settings | All widgets recolor correctly (theme broadcast works) |
| 7 | Change unit system in settings | Physical attributes label in Details updates (settings broadcast works) |
