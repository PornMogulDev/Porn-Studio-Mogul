<span style="color:grey"><i>Written on version ???. <b>Partially</b> updated on version 0.6.0</i></span>  
...
This is the general info about the character.
...
* Alias, and not their 'real' name, which should be a thing at some point. Not much to say, its generation should improve with time, as well as allowing the player to give talent a custom alias for their productions, if they accept.  
After adding nationalities and the extra ethnicities, the aliases pool structure had to be changed, and a proper division between all the different possibilities has not yet been established. I went with divisions by nationality and ethnicity, and then by gender, as such:  
    ...
    "us_latin": {}  
    ...  
But this is way too detailed for aliases, as most aliases are used no matter ethnicity, and nationality has even less of an influence, other than on tokens that explicitly state a nationality, or in special cases, like the Japanese aliases.
...
* Ethnicities are meant to represent 'sexualized populations' that the viewer groups can have preferences for (as well as for particular interracial or same-ethnicity pairings). The four main porn ethnicities (pornicities, if you like) plus some 'sub-groups' for the ones I feel make sense to be distinctly preferenced (Eastern European vs White/Caucasian, East Asian vs Southeast Asian vs South Asian...).
...
* Stamina determines the runtime, influenced also by the type of tags, that a talent can participate in before getting tired, and so their performance being penalized in that (whole) scene. If a talent does go over that threshold, it also becomes Fatigued, which is the only 'status effect' present in the game at this point and will make them perform worse while it lasts. It will last more, and have higher penalties, the more it is pushed and the talent isn't allowed to rest. Talent will start refusing bookings once it reaches 85, which maps to 'High' Fatigue in the view.  Travelling between regions will directly incur Fatigue.
