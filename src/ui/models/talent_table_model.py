...
class TalentTableModel(QAbstractTableModel):
    ...
    def data(self, index: QModelIndex, role: int):
        ...
        if role == Qt.ItemDataRole.DisplayRole:
            ...
            if col == 6: return item.base_location
            if col == 7: return item.effective_location
            if col == 8: return item.dick_size
            if col == 9: return item.cup_size
            if col == 10: return item.performance
            if col == 11: return item.acting
            if col == 12: return item.dom
            if col == 13: return item.sub
    ...
    def _get_or_create_viewmodel(self, row: int) -> Optional[TalentViewModel]:
        ...
        vm = TalentViewModel(
            ...
            orientation=format_orientation(talent_obj.orientation_score, talent_obj.gender), ethnicity=talent_obj.ethnicity, 
            nationality=talent_obj.nationality, base_location=talent_obj.base_location,
            effective_location=cache_item.effective_location,
            ...
            _age_sort=talent_obj.age, _orientation_sort=talent_obj.orientation_score, _nationality_sort=talent_obj.nationality or "", 
            _base_location_sort=talent_obj.base_location or "",
            _effective_location_sort=cache_item.effective_location or "",
    ...
    def sort(self, column: int, order: Qt.SortOrder):
        ...
        def get_sort_key(row_index: int):
            ...
            if column == 6: return vm._base_location_sort.lower()
            if column == 7: return vm._effective_location_sort.lower()
            if column == 8: return vm._dick_size_sort
            if column == 9: return vm._cup_size_sort
            if column == 10: return vm._performance_sort
            if column == 11: return vm._acting_sort
            if column == 12: return vm._dom_sort
            if column == 13: return vm._sub_sort
            if column == 14: return vm._stamina_sort
            if column == 15: return vm._popularity_sort
            if column == 16: return vm._demand_sort
            return 0
