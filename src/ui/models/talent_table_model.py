...
class TalentTableModel(QAbstractTableModel):
    ...
    def _get_or_create_viewmodel(self, row: int) -> Union[TalentViewModel, None]:
        ...
        raw_item = self.raw_data[row]
        unit_system = self.settings_manager.get_setting("unit_system", "imperial")
        
        demand_val = None
        cache_item = None
        
        if isinstance(raw_item, CastingTalentCache):
            cache_item = raw_item
            demand_val = cache_item.demand
        elif isinstance(raw_item, TalentFilterCache):
            cache_item = raw_item
        elif isinstance(raw_item, dict): # Legacy support
            talent_obj, demand_val = raw_item['talent'], raw_item['demand']
            # On-demand calculation for legacy
            from utils.formatters import get_fuzzed_skill_range
            cache_item = TalentFilterCache(
                talent_db=talent_obj,
                perf_range=get_fuzzed_skill_range(talent_obj.performance, talent_obj.experience, talent_obj.id),
                act_range=get_fuzzed_skill_range(talent_obj.acting, talent_obj.experience, talent_obj.id),
                stam_range=get_fuzzed_skill_range(talent_obj.stamina, talent_obj.experience, talent_obj.id),
                dom_range=get_fuzzed_skill_range(talent_obj.dom_skill, talent_obj.experience, talent_obj.id),
                sub_range=get_fuzzed_skill_range(talent_obj.sub_skill, talent_obj.experience, talent_obj.id),
        ...
        vm = TalentViewModel(
            ...
            location=talent_obj.base_location,
            ...
            performance=format_skill_range(perf_range),
            acting=format_skill_range(act_range),
            dom=format_skill_range(dom_range),
            sub=format_skill_range(sub_range),
            stamina=format_skill_range(stam_range),
            popularity=str(cache_item.popularity),
            demand=f"${demand_val:,}" if self.mode == 'casting' and demand_val is not None else "N/A",
            _age_sort=talent_obj.age,
            _orientation_sort=talent_obj.orientation_score,
            _nationality_sort=talent_obj.nationality or "",
            _location_sort=talent_obj.base_location or "",
            ...
            _popularity_sort=cache_item.popularity,
            _demand_sort=demand_val if demand_val is not None else -1
        )
    ...
    def sort(self, column: int, order: Qt.SortOrder):
        ...
        def get_sort_key(row_index: int):
            ...
            if column == 15 and self.mode == 'casting': return vm._demand_sort
            return 0
