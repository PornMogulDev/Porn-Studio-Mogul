...
class TalentFilterPanel(QWidget):
    """
    A reusable QWidget panel for advanced talent filtering. It is a "dumb"
    component responsible only for displaying data and capturing user input.
    All logic is handled by the TalentFilterPanelPresenter.
    """
    # --- Public API and Internal Signals for Presenter ---
    ...
    def __init__(self, settings_manager, ethnicities_hierarchy: dict, cup_sizes: list, nationalities: list, locations_by_region: dict, go_to_categories: list, parent=None):
    ...
    def setup_ui(self):
        ...
        self.apply_button = QPushButton("Apply"); button_layout.addWidget(self.apply_button)
        self.reset_button = QPushButton("Reset"); button_layout.addWidget(self.reset_button)
        main_layout.addLayout(button_layout)

    def connect_signals(self):
        self.go_to_only_checkbox.stateChanged.connect(lambda state: self.go_to_toggled.emit(state == Qt.CheckState.Checked.value))
    ...
    def update_dick_size_filter_ui(self):
        ...
        if self.unit_system == 'metric':
            if label: label.setText("Dick Size (cm):")
            self.dick_range.set_range(inches_to_cm(0), inches_to_cm(20))
        else:
            if label: label.setText("Dick Size (in):")
