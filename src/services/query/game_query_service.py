...
class GameQueryService:
    ...
    SESSION MANAGEMENT PATTERN (Read-Only):
    -----------------------------------------
    1. Store session_factory, NOT a session instance.
    2. Each public method creates its own session using a 'with' statement.
    3. The 'with' statement ensures the session is automatically closed,
       releasing the database connection. This is the idiomatic and safest
       way to handle sessions for both read and write operations.
    
    Example Pattern:
        def public_method(self, ...):
            with self.session_factory() as session:
                # Do query work using 'session'
                result = session.query(...).all()
                return [r.to_dataclass(...) for r in result]
    """
