...
class ShootResultsCalculator:
    """
    Calculates post-shoot outcomes for talents (stamina, fatigue, skills,
    stress) and provides a pre-shoot estimation function for fatigue gain
    used in availability validation.
    """
    ...
    def __init__(self, data_manager: DataManager, config: SceneCalculationConfig,
        ...
        """
        Initializes the calculator with its dependencies.

        Args:
            data_manager: Provides access to game data (e.g., tag definitions).
            config: Contains all configuration constants for calculations.
            role_perf_calculator: Used to determine role-specific stamina costs.
            stress_calculator: Used to determine talent stress gain.
        """
    ...
    def calculate_talent_outcomes(
        ...
        """
        Calculates all per-talent effects from shooting a scene. This is the
        primary method for determining the consequences of a shoot on the cast.

        Args:
            scene: The scene that was shot.
            talents: A list of all participating talent dataclasses.
            active_policies: A list of active studio policies.
            bloc_context: A dictionary with contextual data from the shooting
                          bloc, must contain: `location_def`,
                          `craft_services_efficiency`, `crew_assignments`,
                          and `health_safety_score`.
        Returns:
            A list of TalentShootOutcome DTOs, one for each talent.
        """
    ...
    def estimate_fatigue_gain(self, talent: Union[Talent, TalentDB], scene: Scene, vp_id: int) -> int:
        ...
        """
        Estimates the fatigue gain for a talent in a potential role.
        This is a "what-if" calculation used for pre-booking validation. It does
        not consider the talent's current fatigue level, only the isolated strain
        of this specific role. It also does not rely on `scene.final_cast`.

        Args:
            talent: The talent being considered.
            scene: The scene the role is in.
            vp_id: The virtual performer ID of the role.
        Returns:
            An integer fatigue gain (0-100).
        """
    ...
    def _calculate_stamina_cost_for_role(self, vp_id: int, scene: Scene) -> float:
        ...
        # Use expanded segments to handle tags that are containers for other tags.
    ...
    def _calculate_fatigue(self, talent: Talent, stamina_cost: float) -> FatigueResult | None:
        ...
        # The amount of fatigue gained is proportional to how much the stamina
        # pool was overdrawn. Overdrawing by 100% (cost is 2x pool) results
        # in 100 fatigue gain.
        overdraw_ratio = (stamina_cost - max_stamina) / max_stamina
        fatigue_gain = min(100, int(overdraw_ratio * 100))
    ...
    def _calculate_skill_gain(self, talent: Talent, runtime_minutes: int) -> Tuple[float, float, float]:
        ...
        """
        Calculates potential skill gains for Performance, Acting, and Stamina
        based on scene runtime, with diminishing returns for higher skill levels.
        """
        base_rate = self.config.skill_gain_base_rate
        curve_steepness = self.config.skill_gain_curve_steepness

        def get_gain(skill_value):
            # The closer a skill is to 100, the smaller the gain.
            # The curve_steepness exponent controls how sharply the gains fall off.
    ...
    def _calculate_ds_skill_gain(self, talent: Talent, scene: Scene, disposition: str) -> Tuple[float, float]:
        ...
        """
        Calculates Dom/Sub skill gain based on scene dynamic and talent disposition.
        A talent with a "Dom" disposition will gain more Dom skill in a Dom-focused
        ...
        # Determine the scene's focus (e.g., level 3 is 100% Dom, 0% Sub)
        ...
        # Apply multiplier if talent's disposition matches the focus
        if disposition == "Dom":
        ...
        # Renormalize focus scores to ensure total gain is consistent
        total_focus = dom_focus + sub_focus
        if total_focus > 0:
    ...
    def _calculate_experience_gain(self, talent: Talent, runtime_minutes: int) -> float:
        """
        Calculates experience gain, with diminishing returns as talent
        approaches the maximum experience level.
        """

        base_rate = self.config.exp_gain_base_rate
        curve_steepness = self.config.exp_gain_curve_steepness

        # The closer experience is to 100, the smaller the gain.
