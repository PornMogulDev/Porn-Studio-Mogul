...
class BudgetEfficiencyCalculator:
    ...
    def calculate_efficiency(self, department_def: Dict, budget: int, total_bloc_budget: int, visual_style_def: Dict) -> float:
        ...
        # FIX: Do this BEFORE the min_budget check so we have a base value to penalize
        ...
        # 2. Apply Penalty for under-funding min requirement
        # FIX: Instead of returning a flat constant (which might be higher than the calculated curve),
        # we apply a harsh multiplier to the calculated efficiency.
        if budget < min_budget:
            efficiency *= self.config.budget_min_penalty_multiplier

        # 3. Apply "Over-production" Penalty for Specific Styles
        ...
        # 4. Apply Allocation Percentage Mismatch Penalty
    ...
    def _calc_linear(self, soft_cap: float, budget: int) -> float:
        if soft_cap == 0: return 1.0 # Avoid division by zero
        if budget <= soft_cap:
